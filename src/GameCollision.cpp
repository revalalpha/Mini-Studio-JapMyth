...
void collision_Barrier_Boss(Barrier* go1, Samurai* go2)
{
    go2->destroy();
}
...
void collision_Boss_PlayerProjectile(Samurai* go1, Fireball* go2)
{
    go1->takeDamage(1);
    go2->destroy();
}
void collision_Ship_Boss(PlayerShip* go1, Samurai* go2)
{
    go1->takeDamage(1);
}
...
void Game::onCollision(IGameObject* go1, IGameObject* go2)
{
	...
	HANDLE_COLLIDE(Barrier, Samurai, BARRIER_TYPE, ENEMY_TYPE, collision_Barrier_Boss)
	...
	HANDLE_COLLIDE(Samurai, Fireball, ENEMY_TYPE, PLAYERprojectile_TYPE, collision_Boss_PlayerProjectile)
	HANDLE_COLLIDE(PlayerShip, Samurai, PLAYERSHIP_TYPE, ENEMY_TYPE, collision_Ship_Boss)
